...
test("rejects duplicates, unsupported methods, and unknown fields", () => {
  const host = new McpHost();
  ready(host);
  assert.equal((host.handle({ jsonrpc: "2.0", id: 2, method: "tools/list" }) as any).result.tools.length, 143);
...
test("browser roots reports binding tiers and preview declination", async () => {
  ...
  assert.equal(byName.instruments.binding, "public");
  assert.equal(byName.instruments.searchable, true);
  assert.equal(byName.sounds.binding, "internal");
