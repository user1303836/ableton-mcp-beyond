...
test("rejects duplicates, unsupported methods, and unknown fields", () => {
  const host = new McpHost();
  ready(host);
  assert.equal((host.handle({ jsonrpc: "2.0", id: 2, method: "tools/list" }) as any).result.tools.length, 139);
...
test("session audio import enforces file authority, TOCTOU re-verification, and guarded cleanup", async () => {
  ...
  assert.equal(((await call(5, "live_audio_import_apply", { transactionId: preview.transactionId, confirmation: "apply", idempotencyKey: "import-toctou" })) as any).result.isError, true);
  writeFileSync(audioPath, Buffer.from("RIFF-fake-audio-bytes"));
  ...
  assert.equal(applied.state, "applied"); assert.equal(applied.result.filePath, realpathSync(audioPath));
...
test("tuning and scale edits verify full state and guardedly undo", async () => {
  ...
  ready(host);
  const call = (id: number, name: string, args: unknown) => host.handleAsync({ jsonrpc: "2.0", id, method: "tools/call", params: { name, arguments: args } });
  const preview = JSON.parse(((await call(2, "live_tuning_preview", { referencePitch: 432, rootNote: 9, scaleName: "Minor", scaleIntervals: [0, 2, 3, 5, 7, 8, 10] })) as any).result.content[0].text);
  assert.equal(preview.impact, "edits-global-tuning-audible"); assert.equal(preview.prior.tuningSystem.referencePitch, 440);
  ...
  assert.equal(tuning.system.referencePitch, 432); assert.equal(tuning.scale.rootNote, 9); assert.equal(tuning.scale.scaleName, "Minor");
  ...
  assert.equal(undone.state, "undone");
  assert.equal(tuning.system.referencePitch, 440); assert.equal(tuning.scale.rootNote, 0); assert.equal(tuning.scale.scaleName, "Major");
...
test("selection, draw mode, clip view, device view, and guarded dialog surface", async () => {
  ...
  const clipViewPreview = JSON.parse(((await call(5, "live_clip_view_preview", { clipRef: "clip:clip-1", gridQuantization: 4, tripletGrid: true, showEnvelope: true })) as any).result.content[0].text);
  ...
  assert.equal(clipView.gridQuantization, 4); assert.equal(clipView.tripletGrid, true); assert.equal(clipView.showEnvelope, true);
  ...
  assert.equal(clipView.gridQuantization, 1); assert.equal(clipView.tripletGrid, false);
...
test("device banks, automation re-enable, comparison save, chain insert, and cross-target moves", async () => {
  ...
  const bankPreview = JSON.parse(((await call(3, "live_device_advanced_preview", { action: "set-bank", ref: "device:utility-1", bank: 2 })) as any).result.content[0].text);
  ...
  assert.equal(bankApplied.state, "applied"); assert.equal((simulator as any).state.tracks[0].devices[0].parameterBank, 2);
  const bankUndone = JSON.parse(((await call(5, "live_undo", { transactionId: bankPreview.transactionId, confirmation: "undo", idempotencyKey: "bank-undo-1" })) as any).result.content[0].text);
  assert.equal(bankUndone.state, "undone"); assert.equal((simulator as any).state.tracks[0].devices[0].parameterBank, 1);
  ...
  const comparePreview = JSON.parse(((await call(9, "live_device_advanced_preview", { action: "save-comparison", ref: "device:utility-1", slot: 1 })) as any).result.content[0].text);
  ...
  assert.equal(compared.state, "applied"); assert.equal((simulator as any).state.tracks[0].devices[0].comparison.activeSide, 1);
...
test("chain color and flags, drum pads, rack properties/actions, and rack view", async () => {
  ...
  const padPreview = JSON.parse(((await call(6, "live_drum_pad_preview", { action: "set", padRef: "drum-pad:rack-1:0", note: 40, solo: true })) as any).result.content[0].text);
  ...
  assert.equal(device.drumPads[0].note, 40); assert.equal(device.drumPads[0].solo, true);
  ...
  assert.equal(padUndone.state, "undone"); assert.equal(device.drumPads[0].note, 36); assert.equal(device.drumPads[0].solo, false);
  ...
  const rackPreview = JSON.parse(((await call(12, "live_rack_preview", { action: "set", rackRef: "device:utility-1", visibleMacroCount: 16 })) as any).result.content[0].text);
  ...
  assert.equal(rackApplied.state, "applied"); assert.equal(device.visibleMacroCount, 16);
  ...
  assert.equal(rackUndone.state, "undone"); assert.equal(device.visibleMacroCount, 8);
  ...
  assert.equal(added.state, "applied"); assert.equal(device.macros.length, 2);
...
test("specialized device families, looper, and simpler sample replacement", async () => {
  ...
  const host = new McpHost(simulator);
  ready(host);
  const drift = { ref: "device:drift-1", parentRef: "track:track-1", name: "Drift", kind: "instrument", className: "DriftDevice", parameters: [], objectIdentity: "simulator:device:drift-1", enabled: true, pitchBendRange: 12, voiceCount: 8, voiceMode: 0 };
  (simulator as any).state.tracks[0].devices.push(drift);
  const looper = { ref: "device:looper-1", parentRef: "track:track-1", name: "Looper", kind: "audio-effect", className: "LooperDevice", parameters: [], objectIdentity: "simulator:device:looper-1", enabled: true, speed: 1, loopLength: 4, tempo: 120, fixedRecordLength: 0, state: 0 };
  ...
  const call = (id: number, name: string, args: unknown) => host.handleAsync({ jsonrpc: "2.0", id, method: "tools/call", params: { name, arguments: args } });
  const driftPreview = JSON.parse(((await call(2, "live_device_specialized_preview", { family: "drift", deviceRef: "device:drift-1", pitchBendRange: 24, voiceCount: 16, voiceMode: 1 })) as any).result.content[0].text);
  ...
  assert.equal(drift.pitchBendRange, 24); assert.equal(drift.voiceCount, 16); assert.equal(drift.voiceMode, 1);
  ...
  assert.equal(drift.pitchBendRange, 12); assert.equal(drift.voiceCount, 8); assert.equal(drift.voiceMode, 0);
  const looperPreview = JSON.parse(((await call(5, "live_looper_preview", { action: "set", deviceRef: "device:looper-1", speed: 2, loopLength: 8, fixedRecordLength: 16 })) as any).result.content[0].text);
  ...
  assert.equal(looper.speed, 2); assert.equal(looper.loopLength, 8); assert.equal(looper.fixedRecordLength, 16);
  ...
  assert.equal(looper.speed, 1); assert.equal(looper.loopLength, 4); assert.equal(looper.fixedRecordLength, 0);
  ...
  assert.equal(simplerApplied.state, "applied"); assert.equal(simpler.samplePath, realpathSync(audioPath));
