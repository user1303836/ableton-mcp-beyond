...
test("visibleToolDescriptors returns schema-bearing descriptors only for visible tools", () => {
  ...
  assert.deepEqual(descriptors.map((tool) => tool.name), ["server_status", "capabilities", "plan_user_journey", "audio_analyze", "audio_compare_reference", "live_status", "live_library_search", "live_project_snapshot_diff", "als_read", "als_lint", "als_diff"]);
