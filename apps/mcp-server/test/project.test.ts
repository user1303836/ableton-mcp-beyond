...
test("reads bounded Live Set metadata and writes a verified colocated backup without reading media", () => {
  ...
  try {
    ...
    const backup = projectBackup(set);
    ...
    assert.deepEqual(projectLimitation("save-as"), { available: false, operation: "save-as", reason: "save-as is not exposed by the Live Remote Script API in this Live version and is not fabricated", extensionPoint: "project.info and project.backup are the supported host-side project operations" });
