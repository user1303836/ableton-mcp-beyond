...
test("remote adapter restores subscriptions with a reset after same-epoch reconnect", async () => {
  ...
  try {
    ...
    const refreshed = await adapter.refreshStatusAsync({ deadlineMs: Date.now() + 1000 }); assert.equal(refreshed.connected, true); await new Promise((resolve) => setTimeout(resolve, 10)); assert.equal(subscriptions, 2); assert.equal(events.length, 3); assert.deepEqual(events.map((event) => (event as { type: string }).type), ["reset", "reset", "reset"]); assert.equal((events[1] as { payload?: { reason?: string } }).payload?.reason, "disconnect");
