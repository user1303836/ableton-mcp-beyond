...
test("a rejected subscribe attempt preserves the active event sequence", async () => {
  ...
  const port = await listen(server); let adapter: RemoteScriptLiveAdapter | undefined;
  try { adapter = await RemoteScriptLiveAdapter.connect({ host: "127.0.0.1", port, secret, timeoutMs: 500 }); const events: unknown[] = []; adapter.subscribe((event) => events.push(event)); await adapter.invokeAsync({ operation: "subscribe", args: { types: ["transport"] } }, { deadlineMs: Date.now() + 500 }); await assert.rejects(adapter.invokeAsync({ operation: "subscribe", args: { types: ["transport"] } }, { deadlineMs: Date.now() - 1 }), /deadline/); activeSocket?.write(`${JSON.stringify(response("event", { event: { epoch: 1, sequence: 2, type: "transport", payload: { playing: false } } }))}\n`); await new Promise((resolve) => setTimeout(resolve, 10)); assert.equal(events.length, 2); assert.equal((events[1] as { type: string }).type, "transport"); assert.equal(adapter.status().connected, true); }
  finally { await adapter?.close(); await close(server); }
});
