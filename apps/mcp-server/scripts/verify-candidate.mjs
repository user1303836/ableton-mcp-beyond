...
try {
  ...
  const canonicalNodeRange = ">=22 <23 || >=24 <25 || >=25 <26"; const canonicalNodeMajors = [22, 24, 25];
  if (packageMetadata.private !== true || packageMetadata.license !== "MIT" || packageMetadata.engines?.node !== canonicalNodeRange || JSON.stringify(packageMetadata.abletonMcpSupport?.nodeMajors) !== JSON.stringify(canonicalNodeMajors) || manifest.schema !== "ableton-mcp-release/v2" || manifest.package?.name !== packageMetadata.name || manifest.package?.version !== packageMetadata.version || manifest.package?.license !== packageMetadata.license || manifest.package?.private !== true || manifest.roles?.["LICENSE.md"] !== "license" || manifest.build?.nodeRange !== canonicalNodeRange || JSON.stringify(manifest.build?.nodeMajors) !== JSON.stringify(canonicalNodeMajors) || manifest.source?.commit !== metadata.gitSha || manifest.source?.dirty !== false || manifest.distribution?.channel !== "local-npm-tarball" || manifest.distribution?.published !== false || manifest.distribution?.signed !== false || manifest.distribution?.notarized !== false || manifest.distribution?.integrityIsIdentityProof !== false || !/^[a-f0-9]{64}$/.test(manifest.build?.builder?.packageLockSha256 ?? "") || !/^[a-f0-9]{64}$/.test(manifest.build?.builder?.workflowSha256 ?? "") || !/^[a-f0-9]{64}$/.test(manifest.files?.["package.json"] ?? "") || !readFileSync(join(packageRoot, "LICENSE.md")).equals(readFileSync(new URL("../../../LICENSE.md", import.meta.url)))) throw new Error("candidate does not identify the exact clean Git SHA, MIT license, canonical Node policy, package metadata, and local unpublished channel");
