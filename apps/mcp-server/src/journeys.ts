...
export const JOURNEY_CATALOG: readonly JourneyDefinition[] = [
  {
    ...
    stages: [
      ...
      { id: "arrange", announcement: "Optionally previewing and duplicating the exact created section into Arrangement.", impact: "project-mutation", tools: ["live_clip_duplicate_preview", "live_clip_duplicate_apply", "live_undo"], requiredForCore: false, capabilities: ["session.read", "arrangement.write"], operations: ["clip.duplicate", "arrangement.clip.delete"], authorities: [fixed(["live_clip_duplicate_apply"], "apply", "Confirm the exact duplication preview with apply and a new idempotency key."), fixed(["live_undo"], "undo", "Cleanup is available only through the matching duplicate transaction while its created identity and fingerprint remain exact.")], verification: "Read the exact retained Arrangement clip ref, source relationship, position, length, parent, name, fingerprint, and revision.", recovery: "Use guarded undo only for the exact transaction-created Arrangement clip. Arbitrary Arrangement deletion is refused because deleted clip state cannot be reconstructed.", unavailableFallback: "Keep the section in Session and label Arrangement duplication unavailable." },
      { id: "arrange-edit", announcement: "Optionally replanning after duplication to move the retained section and prove transaction-owned temporary cleanup.", impact: "project-mutation", tools: ["live_clip_move_preview", "live_clip_move_apply", "live_arrangement_clip_preview", "live_arrangement_clip_apply", "live_undo"], requiredForCore: false, capabilities: ["session.read", "arrangement.write"], operations: ["arrangement.clip.create", "arrangement.clip.delete", "arrangement.clip.move"], authorities: [fixed(["live_clip_move_apply", "live_arrangement_clip_apply"], "apply", "Confirm each move or temporary create preview independently with apply and a new idempotency key."), fixed(["live_undo"], "undo", "Delete only through the exact transaction that created the temporary clip; no arbitrary delete preview is available.")], verification: "Verify the retained section's exact moved identity and position, and verify transaction-owned temporary cleanup is absent.", recovery: "Undo the exact unchanged move or transaction-owned create. Refuse arbitrary Arrangement deletion and report manual recovery when ownership/fingerprint no longer matches.", unavailableFallback: "Retain the duplicated section at its original position and provide manual move guidance." },
