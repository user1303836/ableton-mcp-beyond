...
export class RemoteScriptLiveAdapter implements LiveAdapter {
  ...
  static async connect(endpoint: Endpoint): Promise<RemoteScriptLiveAdapter> {
    ...
    await adapter.open();
    const result = await adapter.requestAsync({ method: "status" });
  ...
  private requestAsync(fields: Omit<LoopbackRequest, "version" | "id" | "nonce" | "sequence" | "mac">): Promise<unknown> { if (!this.socket || this.socket.destroyed) return Promise.reject(new Error("remote adapter is disconnected")); if (this.pending.size >= MAX_PENDING) return Promise.reject(new Error("remote adapter queue is full")); if (this.sequence >= MAX_SEQUENCE) return Promise.reject(new Error("remote adapter sequence exhausted")); const id = `async-${++this.sequence}`; const unsigned = { version: LOOPBACK_PROTOCOL_VERSION, id, ...fields, nonce: randomBytes(18).toString("base64url"), sequence: this.sequence }; const request = { ...unsigned, mac: mac(this.endpoint.secret, unsigned) }; return new Promise((resolve, reject) => { const timer = setTimeout(() => { this.socket?.destroy(); this.failPending(new Error("remote adapter request timed out")); }, this.endpoint.timeoutMs ?? DEFAULT_TIMEOUT_MS); this.pending.set(id, { resolve, reject, timer }); try { this.socket?.write(`${JSON.stringify(request)}\n`); } catch (error) { this.pending.delete(id); clearTimeout(timer); reject(error); this.socket?.destroy(); } }); }
  private onData(chunk: Buffer): void { this.buffer = Buffer.concat([this.buffer, chunk]); if (this.buffer.length > MAX_FRAME_BYTES) { this.failPending(new Error("remote frame exceeds limit")); this.socket?.destroy(); return; } while (true) { const index = this.buffer.indexOf(10); if (index < 0) return; const frame = this.buffer.subarray(0, index); this.buffer = this.buffer.subarray(index + 1); if (frame.length === 0) continue; if (frame.length > MAX_FRAME_BYTES) { this.failPending(new Error("remote frame exceeds limit")); this.socket?.destroy(); return; } try { const response = JSON.parse(frame.toString("utf8")) as LoopbackResponse; this.onResponse(response); } catch { this.failPending(new Error("malformed remote response")); this.socket?.destroy(); return; } } }
  private onResponse(response: LoopbackResponse): void { if (!response || response.version !== LOOPBACK_PROTOCOL_VERSION || typeof response.id !== "string" || typeof response.mac !== "string") throw new Error("invalid remote response"); const unsigned = { ...response } as Partial<LoopbackResponse>; delete unsigned.mac; const expected = Buffer.from(mac(this.endpoint.secret, unsigned)); const received = Buffer.from(response.mac); if (expected.length !== received.length || !timingSafeEqual(expected, received)) throw new Error("remote response authentication failed"); if (response.result && typeof response.result === "object" && "event" in (response.result as Record<string, unknown>)) { const event = (response.result as { event?: unknown }).event; if (!event || typeof event !== "object" || !Number.isSafeInteger((event as { sequence?: unknown }).sequence) || ((event as { sequence: number }).sequence <= 0) || typeof (event as { type?: unknown }).type !== "string" || !EVENT_TYPES.has((event as { type: string }).type)) throw new Error("invalid remote event"); for (const listener of this.listeners) listener(event as LiveEvent); return; } const pending = this.pending.get(response.id); if (!pending) throw new Error("unknown or duplicate remote response"); this.pending.delete(response.id); clearTimeout(pending.timer); if (response.ok) pending.resolve(response.result); else pending.reject(new Error(response.error ?? "remote request failed")); }
