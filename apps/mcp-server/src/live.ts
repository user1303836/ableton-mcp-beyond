...
export const LIVE_CAPABILITIES = [
  ...
  "mixing", "transport", "subscriptions", "reconnect",
] as const;
...
export interface LiveSnapshot { set: { ref: LiveRef; name: string; tempo: number; playing: boolean; position: number; loop: { enabled: boolean; start: number; length: number }; }; tracks: Track[]; scenes: Scene[]; }
...
function createSimulatorState(): LiveSnapshot {
  ...
  return { set: { ref: ref("set", "set-1"), name: "Simulator Set", tempo: 120, playing: false, position: 0, loop: { enabled: false, start: 0, length: 4 } }, tracks: [track], scenes: [{ ref: ref("scene", "scene-1"), name: "Scene 1", index: 0 }] };
}
...
export class DeterministicLiveSimulator implements LiveAdapter {
  ...
  get(objectRef: LiveRef): unknown {
    ...
    for (const track of this.state.tracks) {
      ...
      const device = track.devices.find((item) => item.ref === objectRef);
      if (device) return structuredClone(device);
    }
    return undefined;
  }
  ...
  private find(objectRef: LiveRef): Track | Clip | Device | undefined { for (const track of this.state.tracks) { if (track.ref === objectRef) return track; const clip = track.clips.find((item) => item.ref === objectRef); if (clip) return clip; const device = track.devices.find((item) => item.ref === objectRef); if (device) return device; } return undefined; }
