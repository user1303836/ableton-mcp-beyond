...
function verifyReleasePackage(packageRoot: string, allowDirty: boolean): { manifest: ReleaseManifest; manifestSha256: string } {
  ...
  // Known prior runtime policy remains verifiable for receipt-bound status,
  // repair and rollback. It cannot authorize a fresh install or upgrade.
  const currentRuntime = manifest.build?.nodeRange === NODE_ENGINE_RANGE && JSON.stringify(manifest.build?.nodeMajors) === JSON.stringify(SUPPORTED_NODE_MAJORS);
  const priorRuntime = manifest.build?.nodeRange === ">=22 <23 || >=24 <25 || >=25 <26" && JSON.stringify(manifest.build?.nodeMajors) === "[22,24,25]";
  const currentPolicy = manifest.schema === "ableton-mcp-release/v2" && manifest.package.license === "MIT" && manifest.distribution.channel === "local-npm-tarball" && manifest.distribution.integrityIsIdentityProof === false && manifest.roles?.["LICENSE.md"] === "license" && manifest.files?.["LICENSE.md"] === MIT_LICENSE_SHA256 && (currentRuntime || priorRuntime);
  ...
  if (packageMetadata.name !== manifest.package.name || packageMetadata.version !== manifest.package.version || packageMetadata.license !== manifest.package.license || packageMetadata.private !== manifest.package.private || (currentPolicy && (packageMetadata.engines?.node !== manifest.build.nodeRange || JSON.stringify(packageMetadata.abletonMcpSupport?.nodeMajors) !== JSON.stringify(manifest.build.nodeMajors)))) throw new Error("package metadata and release manifest policy disagree");
...
export async function runLifecycle(options: LifecycleOptions): Promise<LifecycleResult> {
  ...
  if (["install", "upgrade"].includes(options.action) && (packageEvidence!.manifest.schema !== "ableton-mcp-release/v2" || packageEvidence!.manifest.build.nodeRange !== NODE_ENGINE_RANGE || JSON.stringify(packageEvidence!.manifest.build.nodeMajors) !== JSON.stringify(SUPPORTED_NODE_MAJORS))) throw new Error("install and upgrade require a current ableton-mcp-release/v2 MIT candidate with current Node policy; older releases are receipt-bound compatibility only");
