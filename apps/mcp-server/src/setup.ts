...
const allowed = new Set(["--output", "--force", "--bridge-host", "--bridge-port", "--secret-file", "--bridge-timeout"]);
...
for (const option of ["--output", "--bridge-host", "--bridge-port", "--secret-file", "--bridge-timeout"]) if (args.filter((arg) => arg === option).length > 1) { console.error(`setup: repeated ${option}`); process.exitCode = 2; }
...
if (process.exitCode !== undefined) {
  ...
  try {
    ...
    const config = bridgeHost !== undefined || bridgePortValue !== undefined || secretFile !== undefined
      ? configForBridge(entrypoint, { host: bridgeHost ?? "127.0.0.1", port: Number(bridgePortValue), secretFile: resolve(secretFile ?? ""), timeoutMs: Number(timeoutValue ?? 5000) }, process.execPath, resolve(output))
