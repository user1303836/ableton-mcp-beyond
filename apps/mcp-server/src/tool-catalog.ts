...
export const TOOL_AVAILABILITY_RULES: readonly AvailabilityRule[] = [
  ...
  { name: "als_read", prereq: { always: true } },
  { name: "als_lint", prereq: { always: true } },
  { name: "als_diff", prereq: { always: true } },
  ...
  { prefix: "live_device_state_recall_", prereq: { capabilitiesAll: ["devices", "parameters", "device.parameter.write"], operationsAll: ["snapshot", "device.parameter.set"] } },
...
export const TOOL_POLICY_RULES: readonly PolicyRule[] = [
  ...
  { name: "als_read", policyClass: "filesystem" },
  { name: "als_lint", policyClass: "filesystem" },
  { name: "als_diff", policyClass: "filesystem" },
  ...
  { name: "live_device_state_save", policyClass: "filesystem" },
  { name: "live_device_state_recall_preview", policyClass: "read" },
  { name: "live_device_state_recall_apply", policyClass: "edit" },
...
const toolDescriptors = [
  ...
  {
    name: "live_batch_preview",
    description: "Read-only preflight for a compound batch: an ordered, bounded list (at most 32) of composable operations (mixer.set, device.parameter.set, clip.set, track.rename, scene.rename, track.create, routing.arm) with per-operation preconditions, merged exact prior-state capture for rollback/undo, per-operation policy enforcement, and a total-change summary. One mutation per exact target per batch.",
    inputSchema: { type: "object", properties: { operations: { type: "array", minItems: 1, maxItems: 32, items: { type: "object", properties: { kind: { type: "string", enum: ["mixer.set", "device.parameter.set", "clip.set", "track.rename", "scene.rename", "track.create", "routing.arm"] } }, required: ["kind"], additionalProperties: true } } }, required: ["operations"], additionalProperties: false },
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true },
  },
  {
    name: "live_batch_apply",
    description: "Apply an exact, unexpired batch preview with confirmation and idempotency: operations execute in order with checkpoint fencing, any clean mid-batch refusal rolls completed steps back to their exact captured prior state, and one undo record covers the whole batch.",
    inputSchema: { type: "object", properties: { transactionId: { type: "string", minLength: 1, maxLength: 128 }, confirmation: { type: "string", enum: ["apply"] }, idempotencyKey: { type: "string", minLength: 8, maxLength: 128 } }, required: ["transactionId", "confirmation", "idempotencyKey"], additionalProperties: false },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
  },
  {
  ...
  {
    name: "als_read",
    description: "Parse one owner-authorized .als file offline (no bridge, no running Live) into the versioned semantic Set snapshot: bounded gunzip and hardened XML reading, per-section completeness with explicitly-unavailable live-only fields, privacy profiles strict/collaboration/local, and optional canonical per-clip MIDI note extraction.",
    inputSchema: { type: "object", properties: { path: { type: "string", minLength: 1, maxLength: 4096 }, allowedRoot: { type: "string", minLength: 1, maxLength: 1024 }, profile: { type: "string", enum: ["strict", "collaboration", "local"] }, limit: { type: "integer", minimum: 1, maximum: 200 }, cursor: { type: "string", minLength: 1, maxLength: 4096 }, includeNotes: { type: "boolean" }, maxRecords: { type: "integer", minimum: 1, maximum: 12000 } }, required: ["path", "allowedRoot"], additionalProperties: false },
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
  },
  {
    name: "als_lint",
    description: "Lint one owner-authorized .als file offline: bounded findings-only checks (clips beyond the last locator, empty tracks, missing sample references inside the allowed root, oversized clip counts, duplicate track names, unwarped long samples) with severity and object identity. Never fixes anything.",
    inputSchema: { type: "object", properties: { path: { type: "string", minLength: 1, maxLength: 4096 }, allowedRoot: { type: "string", minLength: 1, maxLength: 1024 } }, required: ["path", "allowedRoot"], additionalProperties: false },
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
  },
  {
    name: "als_diff",
  ...
  {
    name: "live_device_state_save",
    description: "Save a named parameter-state snapshot of one device or rack subtree to an owner-scoped local file: schema-versioned JSON with device class identity, parameter-layout fingerprint, content digest, and a privacy profile (parameter and device names only — no project paths, session refs, or object identities). Refuses to overwrite without overwrite=true.",
    inputSchema: { type: "object", properties: { deviceRef: { type: "string", minLength: 1, maxLength: 256 }, name: { type: "string", minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$" }, directory: { type: "string", minLength: 1, maxLength: 4096 }, overwrite: { type: "boolean" } }, required: ["deviceRef", "name", "directory"], additionalProperties: false },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
  },
  {
    name: "live_device_state_recall_preview",
    description: "Read-only preflight for recalling a saved device-state snapshot onto an exact target device (or interpolating between two states with an explicit 0..1 amount and deterministic quantization rounding). Validates the file schema/digest, fences device class identity and parameter-layout fingerprint (allowPartialLayout opts into partial recall), and lists per-parameter dispositions (applicable, skipped-read-only, skipped-missing, skipped-rebound).",
    inputSchema: { type: "object", properties: { file: { type: "string", minLength: 1, maxLength: 4096 }, targetDeviceRef: { type: "string", minLength: 1, maxLength: 256 }, morphFromFile: { type: "string", minLength: 1, maxLength: 4096 }, morphFromLive: { type: "boolean" }, amount: { type: "number", minimum: 0, maximum: 1 }, allowPartialLayout: { type: "boolean" } }, required: ["file", "targetDeviceRef"], additionalProperties: false },
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true },
  },
  {
    name: "live_device_state_recall_apply",
    description: "Apply an exact, unexpired device-state recall or morph preview with confirmation and idempotency: parameters write through the guarded device-parameter machinery with per-step revision fencing, exact rollback on mid-recall refusal, and exact prior-state restoration through live_undo.",
    inputSchema: { type: "object", properties: { transactionId: { type: "string", minLength: 1, maxLength: 128 }, confirmation: { type: "string", enum: ["apply"] }, idempotencyKey: { type: "string", minLength: 8, maxLength: 128 } }, required: ["transactionId", "confirmation", "idempotencyKey"], additionalProperties: false },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
  },
  {
