...
| Device lifecycle and parameters | insert/delete/enable/move, bounded published parameter set/undo | host + mapper; G; mutation binds an opaque persistent object identity in addition to its ref and hierarchy fence | host/Python/package journey | `phase-6ab-devices-browser-live.txt` | Plug-in UI control is unsupported; latency is unknown unless authoritatively exposed |
