...
- 按节拍时间的 warp 标记读取与原生添加/移动/删除,带集合栅栏与精确
  ...
  证明待完成。
- 排序 Browser 搜索:宿主侧多词项、与顺序无关的词元匹配,带文档化的整数
  得分与命中词元说明,基于有界、epoch 绑定的按根候选缓存(60 秒 TTL、
  显式刷新,每个结果都报告缓存与截断来源);精确子串匹配保留为文档化回退,
  inspect/load 身份围栏不变。复合批量事务(`live_batch_preview/apply`)在
  单个预览/应用/撤销周期内执行有界(≤32)有序的可组合操作允许列表
  (`mixer.set`、`device.parameter.set`、`clip.set`、`track.rename`、
  `scene.rename`、`track.create`、`routing.arm`),按操作执行部署策略,
  每个精确目标仅一个操作,检查点围栏,中途拒绝时精确回滚已完成步骤,并按
  记录的逐步检查点调和丢失的确认。设备参数状态快照
  (`live_device_state_save`、`live_device_state_recall_preview/apply`)把命名
  的带架构版本、经摘要验证的快照文件保存到属主目录,并在类别与布局围栏的
  目标上召回或变形,带逐参数处置、确定性 float64 量化舍入、逐步修订围栏、
  精确回滚与精确的召回前撤销。批量与设备状态变更仅复用已协商的既有注册表
  操作;已在宿主与模拟器层面验证,精确候选的真实 Live 证明待完成。
- 可选只读 Live 资源库数据库搜索(`live_library_search`):属主允许列表的
  `Live-files-*.db`/`Live-plugins-*.db`,以无依赖的 SQLite 读取器只读打开
  (绝不写入、绝不创建日志、拒绝未检查点 WAL),按架构版本失败关闭式设防
  (文件数据库 12300、插件数据库 1,在 Live 12.4.5 macOS 安装上第一手探测),
  标签/类型/来源/排序查询带分页有界结果,插件清单带厂商/格式过滤,路径隐去,
  相似度/重复显式不可用。已在宿主层面用具象数据库夹具并通过读取真实
  Live 12.4.5 数据库副本验证;更新或更旧的架构版本报告结构化不可用而非臆测。
...
- `pcm-analysis/v3`:隐私保护的波形、频谱、时频、瞬态、声道、相位、动态、
  源削波/重建过冲分离与确定性聚合分析。
