...
- 严格的换行分隔 JSON-RPC 双模式 MCP 宿主（旧 `2025-11-25` / 新 `2026-07-28`），提供新版逐请求元数据 / 发现、结构化 JSON 结果及 private / TTL 0 缓存；具有有界帧/并发、
...
- 排序 Browser 搜索:宿主侧多词项、与顺序无关的词元匹配,带文档化的整数
  ...
  记录的逐步检查点调和丢失的确认。调和使用原始操作参数核对执行台账,随后验证
  新鲜身份与状态;仅值相同不是执行证明。应用、撤销及每步派发前都重新检查策略。
  这是顺序执行,不是原子提交。设备参数状态快照
  ...
  受护栏回滚与召回前撤销。应用、补偿与撤销保留独立检查点,验证或恢复失败仍为
  uncertain。嵌套同级索引用于区分重名元素;输出符号链接在写入前拒绝。
  批量与设备状态变更仅复用已协商的既有注册表
...
- 可选只读 Live 资源库数据库搜索(`live_library_search`):属主允许列表的
  ...
  (绝不写入、绝不创建日志、拒绝未检查点 WAL;仅支持 UTF-8,边界检查计入保留页字节,拒绝超出安全精度的整数),按架构版本失败关闭式设防
