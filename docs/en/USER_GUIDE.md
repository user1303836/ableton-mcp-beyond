...
- Audio-clip preview accepts only fields advertised by the exact clip (`availableAudioFields`): gain, pitch, loop, warp enable/mode, and fades where supported. Warp-marker edits use `live_warp_marker_preview/apply`, address markers by beat time, and include collection fencing and guarded undo. Existing take lanes are discoverable and renamable, and `live_audio_import_preview/apply` can create a file-backed audio clip inside one. Lane creation and MIDI lane-clip creation exist in mapper operations but are not advertised by the current public MCP tool schemas. The public LOM exposes no take-lane deletion, audition, or comp-region editing; those remain unavailable.
