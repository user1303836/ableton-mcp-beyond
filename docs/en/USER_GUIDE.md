...
- `live_browser_search` ranks Browser results host-side: multi-term order-independent token matching with word-boundary/prefix boosts, a documented integer score and matched-token explanation per item, and deterministic ties (score, then name, then id). One bounded candidate traversal (at most 100 items) per root is cached for 60 seconds, keyed by the connection epoch, and never persisted; `refresh: true` forces a fresh traversal. Results report `searchedRoots` (roots that contributed candidates — a bound-limited traversal may not reach every requested root), `candidates`, `candidateBoundReached`, `truncated`, `fromCache`, and `cacheAgeSeconds`. `matchMode: "substring"` keeps the legacy exact pass-through. Loading still requires a fresh `live_browser_inspect` result.
...
- `live_library_search` is an opt-in read-only surface over Live's own library database, independent of a Live connection. It requires an explicit absolute `database` path inside an explicit owner `allowlistRoot`, opens the file read-only (never writes, never creates journals, and refuses databases with uncheckpointed WAL frames), and gates on enumerated schema versions shape-probed first-hand on Live 12.4.5 (files database version 12300; plug-ins database version 1 — anything else reports structured `unavailable` with the observed version, never a guess). File queries filter by name/wildcard, tag conjunction (leaf names or full paths such as `Devices|Synthesizer|FM`), content kind, source, and sort (`useCount`, `modified`, `name`) with bounded revision-paged results and honest truncation; `mode: "plugins"` lists the plug-in inventory with vendor/format filters; `mode: "tags"` lists the tag vocabulary. Database paths and raw filesystem paths are redacted, usage counts are opaque numbers, similarity and duplicate queries report explicit unavailability, and any item that cannot be resolved to a browser-identity candidate is marked discovery-only — loadability still requires `live_browser_inspect`.
...
- `als_read`, `als_lint`, and `als_diff` inspect saved `.als` files offline — no bridge and no running Live: bounded gunzip plus hardened XML parsing into the versioned semantic snapshot (live-only fields explicitly marked unavailable), findings-only lint with severity and object identity, canonical per-clip MIDI extraction that feeds `live_key_estimate`, and diffs of two files or a file against an exported bundle through the existing semantic diff engine. File authority requires an operator-supplied `allowedRoot`; provenance is labeled `offline-file`. `.adg`/`.adv` device files are a documented follow-up.

Offline parsing consumes the entire XML document and refuses malformed markup, unknown entities, duplicate attributes, and resource-limit overflows. Comments/CDATA cannot supply fake elements. It reads child-value timing fields and `MidiNoteEvent` notes, keeps Session slot coordinates, and leaves missing mixer/warp/length values unknown; semantic export refuses an unknown clip length rather than inventing one. Optional MIDI extraction is limited to 256 clips / 4,096 notes, applies the selected name-privacy profile, and includes expressive fields in order-independent revisions. Source evidence uses the bytes already read. Lint resolves relative media paths from the Set directory and does not follow symlink/junction ancestors or probe outside `allowedRoot`.

Generative note plans are bounded before expansion (2,048 output notes; Euclidean patterns at most 64 steps). Bassline steps end at chord boundaries; chord octave notation uses C4 = MIDI 60, preserving the first chord's requested register. Key-estimation note sets accept at most 4,096 notes, with start/duration bounded to 1,000,000 beats; ranking is order-independent and confidence labels are heuristics, not calibrated probabilities or permission to execute edits. Track-color and song-settings apply/undo preserve the previewed object/Set identity even when replacement objects have identical values; uncertain undo cannot resume forward apply.
...
- `live_device_state_save` and `live_device_state_recall_preview/apply` — named parameter-state snapshots of one device or rack subtree. Save writes a schema-versioned, digest-verified JSON file into an explicit owner directory (parameter and device names only; never project paths, session refs, or object identities; re-saving requires `overwrite: true`). Recall validates the file, then fences device class identity (`className` with display-name fallback, plus kind) and the recorded parameter-layout fingerprint: a mismatch refuses before any write with a per-parameter incompatibility report, while `allowPartialLayout: true` opts into a partial recall with explicit per-parameter dispositions (`applicable`, `skipped-read-only`, `skipped-missing`, `skipped-rebound`). Morph (`morphFromFile` or `morphFromLive: true` with an explicit 0..1 `amount`) interpolates host-side with documented deterministic float64 quantization rounding — identical inputs and amount always produce identical values; rounding is half-up from the minimum and capped at the last quantization step within the maximum. Nested device/chain/pad segments include sibling indexes to disambiguate duplicate display names without persisting refs. Output symlinks are refused before writing, including with overwrite enabled. Apply writes through the guarded device-parameter machinery with per-step revision fencing, exact rollback of written parameters on a mid-recall refusal, and guarded pre-recall restoration through `live_undo`. Apply, compensation, and undo retain exact dispatch arguments for ledger reconciliation after lost replies; a matching parameter value is not execution evidence. Verification also checks hierarchy identity. These sequential writes are not an atomic whole-device commit; failed verification or compensation remains uncertain.
...
- `live_batch_preview/apply` — one compound transaction over an ordered, bounded list (at most 32) of composable operations: `mixer.set`, `device.parameter.set`, `clip.set`, `track.rename`, `scene.rename`, `track.create` (name/kind/optional bounded index; track colors are not exposed by the negotiated registry), and `routing.arm`. The read-only preview resolves every target, refuses duplicate mutation targets (one operation per exact target per batch) and new-name collisions, enforces the deployment policy per operation (a single denied operation fails the whole batch before any write), and captures the merged exact prior state. Apply executes in order with checkpoint fencing: each step revalidates its preview preconditions against fresh state immediately before dispatch and verifies its exact postcondition after; a clean mid-batch refusal rolls completed steps back to their exact captured prior state and reports the failing operation index, and a lost acknowledgement reconciles the retained original operation and arguments against the bridge execution ledger using the same transaction and key, then verifies fresh identity and state. Matching values alone never establish execution or undo ownership. Deployment policy is rechecked at apply and undo, including before each step. One undo record covers the whole batch: `live_undo` restores the captured prior state in reverse order, and created tracks are deleted only while identity-and-fingerprint-bound to the transaction. Bulk mixer presets — unmute-all, unsolo-all, unarm-all (`routing.arm`), and solo-exclusive — are expressible as a single batch of per-track operations. This is sequential execution with guarded compensation, not an atomic Live-wide commit. Targets must already resolve at preview; a newly created track cannot be referenced by a later step in the same batch. Failed verification or compensation remains uncertain; retry only the original transaction/key in the same host and bridge/Live epoch, or use explicit recovery finalization after inspection.
