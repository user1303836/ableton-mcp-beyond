...
| Specialized device APIs | `live_device_specialized_preview/apply`, `live_looper_preview/apply`, `live_simpler_preview/apply` | Drift modulation-matrix lists (rows), pitch-bend range, voice count/mode; Drum Cell semantic gain; Eq8 edit/global modes, oversampling, and selected band; Hybrid Reverb IR category/file selection plus attack/decay/size/time shaping; Meld engine, unison, mono/poly, and polyphony; plug-in preset discovery/selection and editor-window state (read/write); Looper transport actions (momentary) and speed/loop-length/tempo/fixed-record-length properties (exact undo); Simpler sample replacement with the same file authority as audio import and an inverse-replacement undo. Every family is negotiated per device class and member presence |
...
| Import an audio file into a Session slot | `live_audio_import_preview/apply` | Explicit file authority: allowed root, canonical path, size/type checks, SHA-256 with apply-time re-verification (anti-TOCTOU), and guarded cleanup of the imported clip |
| Work with take lanes | discovery, `live_object_rename` (kind `takeLane`), `live_arrangement_clip_preview/apply` (`takeLaneRef`), `live_audio_import_preview/apply` (`takeLaneRef`) | Read lanes and their clips, create lanes, rename lanes, and create MIDI or file-backed audio clips inside a lane. The public LOM exposes no take-lane deletion, so lane and lane-clip creation is honestly non-undoable; comp-region editing has no public API and stays unavailable |
...
| Arrangement navigation and clips | arrangement discovery; clip create/duplicate/move; transaction-owned cleanup; locators add/delete/rename; take-lane read/create/rename with lane MIDI/audio clip creation | host transaction managers + mapper; G | host/Python/package journey | `phase-5cd-clip-arrangement-live.txt` plus current tests | `USER_GUIDE.md`; arbitrary Arrangement deletion is refused; exact created identity+fingerprint cleanup applies to create/duplicate only, while moves fence source/destination content and use exact inverse-move recovery, never mint deletion authority, and consume any prior cleanup token for a transaction-created source. The public LOM exposes no take-lane deletion and no comp-region editing; lane and lane-clip creation is non-compensatable by design and reported as such |
...
|---|---|
| `audio.warp-marker.read/add/move/delete` | Implemented (warp-marker family shipped on this branch); schemas address markers by beat time — no invented integer IDs |
| `audio.take-lane.read`, `audio.comp.read` | Take-lane discovery/creation/rename and lane clip creation are implemented; comp-region editing stays limited (no public API), so `audio.comp.read` remains reserved |
