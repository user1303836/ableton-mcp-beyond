...
CI は Ubuntu 24.04 で 1 つのクリーンな tarball をビルドし、新規分離
ローカルクローンと新規 `npm ci` からパックを繰り返してバイトを比較
し、正確な Git SHA と tarball SHA-256 を記録してから、すべての Node
22/24/25 Ubuntu 24.04、macOS 15、Windows Server 2025 ジョブで同じ
成果物をインストールします。各候補ジョブは厳密なインベントリ/ハッシュ
を検証し、ライフサイクルプラン/インストール、利用不可アクティベーション、
冪等修復、非所有ロールバック拒否、アンインストールを実行します。
Windows はさらにネイティブ ACL 修復、ジャンクション拒否、保持ファイル
回復、同梱のバージョン 2 マイグレーションをテストします。これらは
ホスト/パッケージ契約のままです。
