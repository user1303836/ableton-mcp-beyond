...
|---|---|---|---|---|
| リリース成果物 | 厳密な 77 パス MIT npm tarball、リリースマニフェスト、ペイロードロール/ハッシュ、ライセンスバイト一致 | `package:verify`、候補および Python バインダー、新規クローンバイト比較 | 正確な SHA のローカル未公開 tarball のみ | `DISTRIBUTION_POLICY.md`。npm `private: true`、未署名・未公証・未公開 |
...
| 署名/公開 | 明示的な利用不可診断とポリシー | パッケージ/候補ポリシーアサーション | 現在のローカル未公開チャネルには適用されない | MIT の権利とは独立。承認された ID と別の公開決定が必要 |
