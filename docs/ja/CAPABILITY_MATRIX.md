...
| トラックの健全性と状態を読む | `live_snapshot`、`live_discover` | グループ関係、可視性、選択メンバーシップ、フリーズ/フォールド状態、暗黙アーム、バックトゥアレンジャー、ソロ経由ミュート、全入出力メーター、パフォーマンス影響、カラーパレットインデックス/RGB(形状が公開する場合)を全トラック行で公開 |
| トラックプロパティ(カラー) | `live_track_properties_preview/apply` | トラックのカラーパレットインデックス(0-69)を正確な事前値アンドゥと書き込み後検証付きで書き込み。形状が公開する場合、トラック行で解決済み RGB 値も報告 |
...
| デバイスパラメータ状態の保存・呼び出し・モーフ | `live_device_state_save`、`live_device_state_recall_preview/apply` | スキーマ版管理された名前付きスナップショットファイル(コンテンツダイジェストとプライバシープロファイル付き — パラメータ名とデバイス名のみ。プロジェクトパス、セッション参照、オブジェクトアイデンティティは含まない)。呼び出しはデバイスクラスアイデンティティ(`className`、フォールバックは表示名、kind 付き)と記録されたパラメータレイアウトフィンガープリントをフェンス。不一致はパラメータごとの非互換レポート付きで書き込み前に拒否。`allowPartialLayout` でパラメータごとのスキップ(読み取り専用、欠落、再バインド)付き部分呼び出しを選択可能。モーフは明示的な 0..1 量で 2 つのスナップショット(またはスナップショットとライブ状態)を補間し、文書化された決定論的 float64 量子化丸めを使用。書き込みはガード付きデバイスパラメータ機構経由で、ステップごとのフェンス、正確なロールバック、正確な呼び出し前状態へのアンドゥ付き |
...
| 複合バッチトランザクション | `live_batch_preview/apply` | 有界(≤32)かつ順序付きの合成可能な操作リスト(`mixer.set`、`device.parameter.set`、`clip.set`、`track.rename`、`scene.rename`、`track.create`、`routing.arm`)を 1 回のプレビュー/適用/アンドゥで実行。操作ごとのポリシー強制、同一ターゲットは 1 操作のみ、チェックポイントフェンス、途中拒否時の完了済みステップの正確なロールバック、失われた ack の記録済みチェックポイントによる reconcile、バッチ全体で 1 つの逆順復元アンドゥレコード。バルクプリセット(unmute-all、unsolo-all、unarm-all、solo-exclusive)を 1 バッチで表現可能 |
...
| 深い Song と Link 状態を読む | `live_song_state` | 可視トラック、任命デバイス、ソング長/開始、拍子、スウィング、オーバーダブ/アレンジメントオーバーダブ、バックトゥアレンジャー、キャプチャ/アンドゥ/リドゥ可否、排他アーム/ソロ、カウントイン中、テンポフォロワー、オートメーション再有効化、Session 録音/オートメーション、クリップトリガーと MIDI 録音クオンタイズ、Ableton Link 有効/スタートストップ同期 — ビート↔SMPTE とループ時間変換付き |
| Song 再生設定を書き込む | `live_song_settings_preview/apply` | グローバル拍子(分子/分母を順序付きで書き込み、部分失敗時は正確にロールバック)、スウィング量(0..1)、クリップトリガークオンタイズ、MIDI 録音クオンタイズ — 形状がセッターを公開する場合のみ書き込み可能。トランスポート編集と同様のリビジョンフェンスと正確なアンドゥ付き。拍子とトリガークオンタイズの変更は再生フィールに即座に影響(プレビューで開示) |
...
| 調を推定 | `live_key_estimate` | デュレーション/ベロシティ加重のピッチクラス相関(Krumhansl-Schmuckler プロファイル、文書化されたモード候補とトーナルセンタータイブレーク)。ランク付き候補とスコア、明示的な信頼度分類、曖昧さフラグ。決定的かつ読み取り専用。曖昧・半音階的・薄弱な素材は推測せず正直に報告 |
...
| クリップのミュート、カラー、ループ、起動 | `live_clip_properties_preview/apply` | 任意のクリップのミュートとカラー。MIDI クリップのループ境界(オーディオループは `live_audio_clip_*`)。起動モード(Trigger/Gate/Toggle/Repeat)、Global センチネルを含むクリップごとの起動クオンタイズ、および公開されている任意のクリップの legato(再生中/トリガー中は拒否)。オーディオクリップの RAM モード。MIDI クリップのベロシティ量 — 各フィールドは正確なクリップが公開する場合のみ書き込み、正確なロールバックとアンドゥ付き |
...
| Browser を検索してアイテムを検査 | `live_browser_search`、`live_browser_roots`、`live_browser_inspect` | ホスト側の複数語ランクマッチ(順序非依存トークン、語境界/前方一致ボーナス、項目ごとの文書化スコアとマッチトークン説明)を、有界で epoch 束縛のルート別候補キャッシュ(60 秒 TTL、明示的 `refresh`、キャッシュ由来と切り捨てを報告)上で実施。完全一致サブストリングは文書化されたフォールバックモードとして維持。タグ/フィルター/類似検索ではない — それらの公開 API は存在しない(下記ライブラリデータベースサーフェスがスキーマ列挙時にタグをカバー)。形状ゲートのルート(sounds、samples、User Library、ユーザーフォルダ、現在のプロジェクト)と、各バインディングの階層を `live_browser_roots` が報告。単一アイテムの `live_browser_inspect` は安定したアイデンティティ、タイプ、来歴、明示的なロード可否を生のファイルシステムパスなしに返す。内部バインディングは安定した公開 LOM API ではない |
| Live ライブラリデータベースを検索(タグ、種別、使用回数、プラグインインベントリ) | `live_library_search` | オプトイン、オーナー許可リスト、読み取り専用(書き込みは一切なし、未チェックポイント WAL は拒否)。Live 12.4.5 で直接シェイププローブしたバージョンでフェイルクローズドにスキーマゲート(ファイルデータベース 12300、プラグインデータベース 1。それ以外は観測バージョン付きの構造化 unavailable を報告)。名前/ワイルドカード + タグAND + 種別 + ソース + ソートフィルタで有界かつリビジョン束縛のページング。ベンダー/フォーマットフィルタ付きプラグインインベントリ。パスは秘匿、使用回数は不透明。類似/重複は明示的に unavailable。discovery-only アイテムはラベル付きで、ロード可否は引き続き `live_browser_inspect` が必要 |
...
|---|---|---|---|---|---|
| MCP トランスポートとホスト | initialize、tools/resources/prompts、stdio JSON-RPC | `host.ts`、`stdio.ts`、`framing.ts`。R/G。有界フレーム、ワーク、レート、キャンセル、順序付け | `host.test.ts`、`stdio.test.ts`、`framing.test.ts`、プロパティ/ベンチマーク | Node 22/24 ホストマトリクスが設定済み。パッケージ fake-Live ジャーニー。正確な SHA 結果が必要 | `DEVELOPER_GUIDE.md`、`OPERATIONS.md`。汎用ミューテーションツールなし |
...
| 生成 MIDI プリミティブ | `live_midi_transform_preview/apply` の `euclidean`、`chord-progression`、`drum-pattern`、`bassline`、`motif-invert`、`motif-retrograde`、`motif-augment`、`motif-diminish` 変換 | `midi-transforms.ts` の純粋な決定論的ジェネレータ: Bjorklund ユークリッドリズム(最大限均一、最初のパルスの正規化、決定論的ローテーション)、ローマ数字または明示的シンボルによるコード進行(仮定ではなく音階から実現される品質、close/drop-2/spread ボイシング、最小移動ボイスリーディング)、文書化されたドラムテンプレート(four-on-the-floor、backbeat、breakbeat、trap hats。グリッド/密度/シードパラメータ)、ルート追従ベースライン(octave、walking、arpeggiated)、モチーフ変換(明示的軸の反転、レトログレード、正確な比率の増大/縮小)。キットマッピングとキーは捏造されない: 明示的引数または Set 発見のコンテキスト(ドラムチェーンノート、ソングスケール)から来て、プレビューで開示される | `midi-generative.test.ts` プロパティカバレッジ(最大均一性、ボイスリーディング単調性、バイト単位の再現性)+ `midi-transform-host.test.ts` プレビュー/適用/アンドゥパス | ホスト/シミュレータ契約。正確な候補の実 Live エビデンスは保留中 | `USER_GUIDE.md`。ジェネレータは duplicate-first のまま。モチーフは update-only |
...
| オフライン .als 検査 | `als_read` / `als_lint` / `als_diff` による保存済み Set のオフライン解析・lint・差分(ブリッジ不要、Live 不要) | `als.ts` の有界 gunzip + 堅牢化 XML リーダー(DOCTYPE/ENTITY 拒否、深度/ノード/属性/テキスト有界)。セマンティック成果物はバージョン付き `project-semantic.ts` パイプラインで組み立て、`offline-file` プロビナンスと明示的に利用不可のライブ専用フィールド付き。オーナー `allowedRoot` 包含 + 正規の通常ファイルチェック。発見専用 lint(重要度とオブジェクトアイデンティティ付き)。`live_key_estimate` と既存セマンティック diff エンジンへ供給するクリップごとの正規 MIDI 抽出 | `als.test.ts`: 合成 fixture、敵対的 XML ガード、プライバシープロファイル、フェイルクローズドホスト経路、バンドル/ファイル混在差分 | シミュレーター/fixture 証拠のみ。実 .als コーパス検証は保留中 | `.adg`/`.adv` デバイスファイルは文書化されたフォローアップ。構造的ヒューリスティックであり汎用 XML パーサーではない。lint は修正を提案しない |
...
| 波形/スペクトル/時間-周波数/トランジェント/位相/ダイナミクス | `pcm-analysis/v3` 集計サマリー | P/R | 決定論的フィクスチャと境界 | パッケージジャーニー | 損失のある集計エビデンス。ソース再構築やマスタリング判定ではない |
...
|---|---|---|---|---|
| リリース成果物 | 許可リストに厳密一致する MIT npm tarball、リリースマニフェスト(ペイロードの列挙は `release-manifest.json` を正とする)、ペイロードロール/ハッシュ、ライセンスバイト等価性 | `package:verify`、候補と Python バインダー、新規クローンバイト比較 | 正確な SHA のローカル未公開 tarball のみ | `DELIVERY.md`。npm `private: true`、未署名、未公証、未公開 |
...
| Node/OS 互換性 | Node 22/24。Ubuntu 24.04、macOS 15、Windows Server 2025 ワークフロー | 完全な Node テストと正確なインストール済み候補。Python 3.11 マッパー | 条件付き。現在のチェック結果を参照 | Linux に Live の主張なし。Windows 11 は Server から継承されない |
